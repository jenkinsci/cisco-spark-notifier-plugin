...
public class MarkdownTestResultService implements TestResultService {
	private static final Logger LOG = Logger.getLogger(MarkdownTestResultService.class.getName());

	private final TestResult testResult;
	private final int maxMessageCharLimit;

	public MarkdownTestResultService(final TestResult testResult, final int maxMessageCharLimit) {
	...
	public String appendMessageWithTestInformation(final String message) {
		...
		String preMessage = StringUtils.isNotEmpty(message) ? message + MessageConst.MARKDOWN_LINE_BREAK : StringUtils.EMPTY;

		TestResultAction testResults = testResult.getTestResultAction();
		StringBuilder titleAndSummary = new StringBuilder(90);

		titleAndSummary.append(MessageFormatUtils.convertToMarkdownLink(MessageConst.TEST_RESULT_URL, MessageFormatUtils.convertToMarkdownBold(MessageConst.TEST_RESULT_TITLE)));
		...
		if (maxMessageCharLimit != 0 && messageWithTestInfo.length() > maxMessageCharLimit) {
			...
			String errorMessage = preMessage + titleAndSummary + MessageConst.MARKDOWN_LINE_BREAK + MessageConst.MESSAGE_TOO_BIG_WITH_FACE;
